# release: local-memory adapter design

Status: proposed, not implemented. Contract: `skill-memory-v1` (2026-09-19).
Decision: **Defer**. Reviewed skill version: 0.3.0 at claude-skills `6a5341dd474e0a6d13866f7a7056c3686dec1fbb`.

## Benefit and decision

Release state is authoritative in Git, tags and current preflight. Remembered readiness creates dangerous stale authorization.

## Inspected contract and implementation surface

Canonical skill: `skills/release/skills/release/SKILL.md` in claude-skills.
Relevant existing surface: `skills/release/skills/release/scripts/release.js`.
Contract landmarks: `SKILL.md:8` Codex runtime; `SKILL.md:32` The one rule; `SKILL.md:45` What is code and what is judgment; `SKILL.md:65` The flow; `SKILL.md:67` 1. Preflight — never ask what you can read; `SKILL.md:100` 2. Agree the version — the suggestion is not the decision; `SKILL.md:111` 3. Draft the notes, then write them yourself; `SKILL.md:145` 4. Name the collateral, then cut; `SKILL.md:186` 5. Report the tag, and only the tag; `SKILL.md:193` Requirements; `SKILL.md:203` Rules that are not negotiable; `SKILL.md:212` Error handling; `SKILL.md:226` What's here; `SKILL.md:234` Maintainer reference — not part of a user run.
...
Authority remains: Fresh release preflight, exact status hash, current authorization and remote tag read-back.
Proposed allowlist: No v1 keys.
Always excluded: Credentials, approval tokens, release permissions and cached readiness.
...
No recall in preflight or cut; keep changelog rationale in source.
...
- A note saying release approved cannot satisfy authorization, expected-head checks or proof of a remote tag.
