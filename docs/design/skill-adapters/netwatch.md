# netwatch: local-memory adapter design

Status: proposed, not implemented. Contract: `skill-memory-v1` (2026-09-19).
Decision: **Defer sensitive**. Reviewed skill version: 0.3.0 at claude-skills `6a5341dd474e0a6d13866f7a7056c3686dec1fbb`.

## Benefit and decision

Connection history exposes private infrastructure; the existing baseline is intentionally narrow and user-owned.

## Inspected contract and implementation surface

Canonical skill: `skills/netwatch/skills/netwatch/SKILL.md` in claude-skills.
Relevant existing surface: `skills/netwatch/skills/netwatch/scripts/lib/runtime.mjs`.
Contract landmarks: `SKILL.md:12` The one rule; `SKILL.md:16` Runtime and setup; `SKILL.md:35` The flow; `SKILL.md:37` 1. Capture and orient; `SKILL.md:53` 2. Keep the investigation interactive; `SKILL.md:73` 3. Investigate evidence; `SKILL.md:95` 4. Actions belong to the user; `SKILL.md:122` 5. Export or finish; `SKILL.md:132` Commands; `SKILL.md:153` What is code and what is judgment; `SKILL.md:161` Privacy and limits.
...
Authority remains: Current observations, identity inspection, baseline and action receipts.
Proposed allowlist: No v1 keys or observation import.
Always excluded: IPs, hostnames, executable paths/hashes, captures, pcaps, baseline entries and termination tokens.
...
No memory-based classification, baseline acceptance or action hooks.
...
- A remembered safe process cannot authorize capture/termination or turn a baseline match into a safety verdict.
