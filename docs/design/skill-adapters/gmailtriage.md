# gmailtriage: local-memory adapter design
...
Decision: **Defer sensitive**. Reviewed skill version: 0.8.1 at claude-skills `6a5341dd474e0a6d13866f7a7056c3686dec1fbb`.

## Benefit and decision

Existing rules and undo receipts already provide durable memory with concrete mailbox semantics. Generic notes cannot safely drive mail mutations.

## Inspected contract and implementation surface

Canonical skill: `skills/gmailtriage/skills/gmailtriage/SKILL.md` in claude-skills.
Relevant existing surface: `skills/gmailtriage/skills/gmailtriage/scripts/lib/rules.mjs`.
Contract landmarks: `SKILL.md:8` Codex runtime; `SKILL.md:42` The one rule; `SKILL.md:46` What is code and what is judgment; `SKILL.md:76` The flow; `SKILL.md:78` 0. Setup — always first, and the only thing safe to run cold; `SKILL.md:93` 1. Fetch the sample, write it verbatim, and `ingest` — never build JSON by hand; `SKILL.md:188` 1b. Audit — every run, before anything else; `SKILL.md:217` 2. Propose — the first run, or any run with mail that has no home; `SKILL.md:251` 2b. Subdivide — when a folder has grown into several things; `SKILL.md:291` 3. Accept rules; `SKILL.md:314` 3b. Reconcile the folders — before anything moves; `SKILL.md:344` 4. Plan — always, every run; `SKILL.md:369` 4b. The retroactive pass — applying new rules to mail already filed; `SKILL.md:389` 5. Apply, then perform exactly what it authorised.
...
Authority remains: Owner rule store, current mailbox plan and undo receipts.
Proposed allowlist: No v1 keys or mailbox import. Any future adapter needs a separate restricted-store design.
Always excluded: All message content, senders, labels, queries, rules, receipts, OAuth data and mailbox topology.
...
Keep all persistence with gmailtriage. Do not mirror rules or use generic memory as a plan input.
...
- A note saying to trash a sender must not become a rule or bypass current plan/authorization checks.
